...
function testGetAge(birthdate, now = Date.now, expected) {
    ...
    console.assert(actual.years === expected.years && actual.months === expected.months && actual.days === expected.days, `Expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`);
}
